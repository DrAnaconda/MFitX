...
//endregion
    ...
    fun ExtractRecords(Argument: Calendar, OffsetArg: Long, Operator: SQLiteDatabase): List<MainRecord> {
        val Results = ArrayList<MainRecord>()
        val SelectArg = CustomDatabaseUtils.CalendarToLong(Argument, true)
        val EndSArg = SelectArg + OffsetArg
        val Records = Operator.query(DatabaseController.MainRecordsTableName, ColumnsForExtraction,
                ColumnNames[1] + " BETWEEN ? AND ?", arrayOf(java.lang.Long.toString(SelectArg), java.lang.Long.toString(EndSArg)), null, null, ColumnNames[1] + " DESC")
        Records.moveToFirst()
        do {
    ...
    fun ExtractRecords(From: Long, To: Long, Operator: SQLiteDatabase): Cursor {
        val Record = Operator.query(DatabaseController.MainRecordsTableName, ColumnsForExtraction,
                ColumnNames[1] + " BETWEEN ? AND ?", arrayOf(java.lang.Long.toString(From), java.lang.Long.toString(To)), null, null, ColumnNames[1])
        Record.moveToFirst()
        return Record
    }
    ...
    fun ExtractFuncOnIntervalCalories(Where: Long, To: Long, Operator: SQLiteDatabase): Cursor {
        val Record = Operator.query(
                ...
                ColumnNames[1] + " BETWEEN ? AND ?", arrayOf(java.lang.Long.toString(Where), java.lang.Long.toString(To)), null, null, ColumnNames[1])
        Record.moveToFirst()
        return Record
    }
